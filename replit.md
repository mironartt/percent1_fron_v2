...
**Mobile Navigation (Remente-style)**: Bottom navigation with centered FAB button. Structure: Главная - Цели - [FAB] - План - Ещё. The FAB is a raised circular button (#4f46e5 indigo, no shadow), containing quick actions for tasks, goals, journal, habits, and AI mentor. FAB overlay uses Teleport to body for correct z-index stacking. MentorFloatingButton is hidden on mobile (< 768px).
...
-   **Subscription Notifications System**: Trial and subscription expiration alerts with `TrialBanner.vue` (4 states: trial_normal, trial_warning, trial_danger, freemium) and `SubscriptionExpiredModal.vue` (for trial_expired and paid_expired). Features: trial banner displayed everywhere in /app/* for trial users, freemium banner only on Dashboard, modals shown once (server-side flag via API `/api/rest/front/app/subscription/modal-shown/`), proper day pluralization (день/дня/дней). Components location: `src/components/subscription/`. Integration with subscriptionStore for state management (isPaidExpired, shouldShowTrialExpiredModal, shouldShowPaidExpiredModal, subscriptionState computed).
...
-   **QuickAddTask.vue**: Quick task creation modal with required goal selection. Features: combobox goal selector (input+dropdown with filtering), compact 3-column dropdown layout for day/priority/time selectors, priority color dot indicator, auto-fill date when "custom" selected. Uses createStep() and scheduleStep() API for backend sync. Stacks vertically on mobile (< 480px).
-   **BottomNavigation.vue**: Mobile bottom navigation with integrated FAB. Structure: Главная - Цели - [FAB] - План - Ещё. FAB is centered and raised above the navigation bar (Remente-style). FAB menu includes: Быстрая задача, Новая цель, Запись в дневник, Добавить привычку, AI ментор. All FAB modals (QuickAddTask, AddGoalModal, AddHabitModal, JournalEntry) are now managed within BottomNavigation.
-   **FloatingActionButton.vue**: Legacy standalone FAB component (now superseded by BottomNavigation integration on mobile).
