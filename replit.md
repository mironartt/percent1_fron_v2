...
- **Goal Details Page**: Mobile-first redesign with compact sticky header (back button, editable goal title, sphere badge, progress bar, settings button). Step cards display priority/time/date meta-info with checkbox for quick completion. FAB button for adding steps. Bottom Sheet modal for step editing with Quick Actions (Complete, Schedule, Delete) and tabbed structure (Main | Parameters). Fields unified to `timeEstimate` with `formatTimeEstimate()` helper. Auto-save syncs changes with backend.
