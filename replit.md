...
-   **Landing Version 5 (now main `/`)**: Combined version with version4's 5-step user journey plus main landing's trust-building elements: hero with animated compound growth chart, header with login/register, "Проверено временем" legends section (Franklin, Clear, Buffett, da Vinci), dynamic pricing section with term selector, and enhanced footer with legal links and company info. Old landing moved to `/land/old_v1`.
