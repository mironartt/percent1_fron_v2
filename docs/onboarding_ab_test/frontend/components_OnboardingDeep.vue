...
const finishOnboarding = async () => {
  ...
  try {
    ...
    await store.updateSSPData(sspData)

    // 2. Save accepted goals
    ...
    await store.saveGoalsToBackend()

    // 3. Set finish_onboarding = true
    await store.updateOnboardingData({ finish_onboarding: true })
    store.user.finish_onboarding = true
    ...
    // 5. Award XP
    if (store.addXP) {
      await store.addXP(50, 'Завершение онбординга')
    }
